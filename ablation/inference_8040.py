...
# 【修改点 1】修改输出结果的文件名
OUTPUT_FILE = os.path.join(WORK_DIR, 'inference_results_8040.pkl')
...
class RadarTransUNet1D(nn.Module):
    def __init__(self, num_classes, d_model=64, nhead=4, num_layers=2):
        ...
        self.pos_encoder = nn.Parameter(torch.randn(1, 80, d_model) * 0.1)
    ...
    def forward(self, x):
        ...
        x_up = self.up_early(x) 
        e1 = self.enc1(x_up)    
        
        t_in = e1.permute(0, 2, 1) 
...
print("Loading TransUNet model (80-40)...")
model = RadarTransUNet1D(num_classes=decoder.num_classes).to(DEVICE)
# 【修改点 2】注意加载的是 80-40 专属的权重名
model.load_state_dict(torch.load(os.path.join(WORK_DIR, 'transunet_model_8040.pth'), map_location=DEVICE))
model.eval()
