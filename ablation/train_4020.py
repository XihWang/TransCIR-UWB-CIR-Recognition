...
class RadarTransUNet1D(nn.Module):
    def __init__(self, num_classes, d_model=64, nhead=4, num_layers=2):
        ...
        # --- 3. 后置压缩 (Late Compression: 40 -> 20 Bins) ---
        self.pool_late = nn.MaxPool1d(2) # 强行将 40 Bins 压缩一半
        self.dec_late = nn.Sequential(nn.Conv1d(d_model, 16, 3, padding=1), nn.BatchNorm1d(16), nn.ReLU())
        
        # --- 4. 分类头 (基于压缩后的 20 Bins 进行决策) ---
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(16 * 20, 128), # 16通道 * 20 bins
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def forward(self, x):
        ...
        # === Transformer 全局交互 (40 Bins) ===
        t_in = e2.permute(0, 2, 1) # [B, 40, d_model]
        t_out = self.transformer(t_in + self.pos_encoder) # [B, 40, d_model]
        t_out = t_out.permute(0, 2, 1) # [B, d_model, 40]
        
        # === 后置空间压缩 ===
        p_out = self.pool_late(t_out) # [B, d_model, 20]
        d_out = self.dec_late(p_out)  # [B, 16, 20]
...
print(f"Starting Training Model (40-20 Late Compression) on {DEVICE}...")
model.train()
...
# 更改模型保存名字
torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'transunet_model_4020.pth'))
print("TransUNet Model (40-20) Saved.")