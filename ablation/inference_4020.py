...
# 【修改点 1】修改输出结果的文件名
OUTPUT_FILE = os.path.join(WORK_DIR, 'inference_results_4020.pkl')
...
print("Loading TransUNet model (40-20)...")
model = RadarTransUNet1D(num_classes=decoder.num_classes).to(DEVICE)
# 【修改点 2】注意加载的是 40-20 专属的权重名
model.load_state_dict(torch.load(os.path.join(WORK_DIR, 'transunet_model_4020.pth'), map_location=DEVICE))
model.eval()
