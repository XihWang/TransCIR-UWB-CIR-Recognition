...
class RadarTransUNet1D(nn.Module):
    def __init__(self, num_classes, d_model=64, nhead=4, num_layers=2):
        ...
        # --- 3. 后置极度压缩 (Late Compression: 40 -> 10 Bins) ---
        self.pool_late = nn.MaxPool1d(4) # 步长为4，强制将 40 压缩为 10
        self.dec_late = nn.Sequential(nn.Conv1d(d_model, 16, 3, padding=1), nn.BatchNorm1d(16), nn.ReLU())
        
        # --- 4. 分类头 (基于极度压缩的 10 Bins) ---
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(16 * 10, 128), # 16通道 * 10 bins
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def forward(self, x):
        ...
        # === 后置极度压缩 ===
        p_out = self.pool_late(t_out) # 压扁到 10 Bins: [B, d_model, 10]
        d_out = self.dec_late(p_out)  # [B, 16, 10]
...
print(f"Starting Training Model (40-10 Late Extreme Compression) on {DEVICE}...")
model.train()
...
# 更改模型保存名字
torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'transunet_model_4010.pth'))
print("TransUNet Model (40-10) Saved.")