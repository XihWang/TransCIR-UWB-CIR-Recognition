...
# 【修改点 1】修改输出结果的文件名
OUTPUT_FILE = os.path.join(WORK_DIR, 'inference_results_4080.pkl')
...
class RadarTransUNet1D(nn.Module):
    def __init__(self, num_classes, d_model=64, nhead=4, num_layers=2):
        ...
        self.up_late = nn.ConvTranspose1d(d_model, 16, kernel_size=2, stride=2) 
    ...
    def forward(self, x):
        ...
        u_out = self.up_late(t_out) 
        d_out = self.dec_late(u_out) 
...
print("Loading TransUNet model (40-80)...")
model = RadarTransUNet1D(num_classes=decoder.num_classes).to(DEVICE)
# 【修改点 2】加载 40-80 专属权重
model.load_state_dict(torch.load(os.path.join(WORK_DIR, 'transunet_model_4080.pth'), map_location=DEVICE))
model.eval()
