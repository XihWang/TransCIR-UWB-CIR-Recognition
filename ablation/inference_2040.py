...
# 【修改点 1】修改输出结果的文件名
OUTPUT_FILE = os.path.join(WORK_DIR, 'inference_results_2040.pkl')

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# ================= 【消融实验】同步 Bottleneck=20 结构 =================
class RadarTransUNet1D(nn.Module):
    def __init__(self, num_classes, d_model=64, nhead=4, num_layers=2):
        ...
        # 【删除了 pool2，保留 20 的分辨率】
        
        # --- 2. Bottleneck (Transformer 长度变为 20) ---
        self.pos_encoder = nn.Parameter(torch.randn(1, 20, d_model) * 0.1)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=nhead, dim_feedforward=d_model*2, dropout=0.1, batch_first=True
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        
        # --- 3. Decoder (只需要一次上采样: 20 -> 40) ---
        self.up2 = nn.ConvTranspose1d(d_model, 16, kernel_size=2, stride=2) # 20 -> 40
    ...
    def forward(self, x):
        ...
        # === Transformer 瓶颈阶段 (20 Bins) ===
        t_in = e2.permute(0, 2, 1) # [B, 20, 64]
        t_out = self.transformer(t_in + self.pos_encoder) # [B, 20, 64]
        t_out = t_out.permute(0, 2, 1) # [B, 64, 20]
        
        # === 解码阶段 (只有一次上采样与拼接) ===
        d2 = self.up2(t_out)               # 放大到 [B, 16, 40]
        c2 = torch.cat([e1, d2], dim=1)    # 任意门拼接: 32(来自e1) + 16 = 48
...
print("Loading TransUNet model (20-40)...")
model = RadarTransUNet1D(num_classes=decoder.num_classes).to(DEVICE)
# 【修改点 2】注意加载的是 20-40 专属的权重名
model.load_state_dict(torch.load(os.path.join(WORK_DIR, 'transunet_model_2040.pth'), map_location=DEVICE))
model.eval()
