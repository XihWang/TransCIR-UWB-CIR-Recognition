...
class RadarTransUNet1D(nn.Module):
    ...
    def forward(self, x):
        ...
        e2 = self.enc2(e1)      # [B, 64, 40]
        
        # === Transformer 瓶颈阶段 (40 Bins) ===
        t_in = e2.permute(0, 2, 1) # 转为 [B, 40, 64]
        t_out = self.transformer(t_in + self.pos_encoder) # [B, 40, 64]
        t_out = t_out.permute(0, 2, 1) # 转回 [B, 64, 40] 准备展平
...
print(f"Starting Training Model (40-40 No Compression) on {DEVICE}...")
model.train()
...
# 更改模型保存名字
torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'transunet_model_4040.pth'))
print("TransUNet Model (40-40) Saved.")